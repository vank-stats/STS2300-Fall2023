...
**Answer:** There are 110,495 observations and 22 variables
...
```{r}
results <- Batting %>%
  filter(teamID == "MIN", H >= 200)
str(results)
table(results$playerID)
```

**Answer:** There are 13 rows in results. However, there are only five different players.
...
```{r}
BOS <- filter(Batting, teamID == "BOS") %>%
  summarize(Seasons = n(), .by = playerID)
```
...
```{r}
BOS_NYA <- inner_join(BOS, NYA, by = join_by(playerID))
str(BOS_NYA)
```
