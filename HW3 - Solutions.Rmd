...
# Graphing - Hedgehogs (23 points total)

```{r, out.width = '50%', fig.align = 'center', echo = FALSE, fig.cap = 'European Hedgehogs [Image Source](https://commons.wikimedia.org/wiki/File:European_Hedgehog_Mother_and_Child.jpg)'}
knitr::include_graphics('https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/European_Hedgehog_Mother_and_Child.jpg/1600px-European_Hedgehog_Mother_and_Child.jpg?20110817144558')
```

A [recent study](https://www.mdpi.com/2076-2615/13/4/626#) looked at lifespans and causes of death of European Hedgehogs. The authors of the paper appear to have created their graphs using `ggplot2`. You will be recreating each of the three graphs in this image and then eventually putting them together like the authors did.
...
1. (7 points) Use the data to create **graph B**. Pay attention to each of the following when making your graph:
...
```{r}
B <- ggplot(hedgehogs, aes(x = Sex, fill = TrafficDeath)) +
  geom_bar() +
  theme_classic() +
  labs(title = "B - Deaths by sex and cause",
       fill = "Traffic Death", y = "Count") +
...
hedgehog_sum <- hedgehogs %>%
  ...
  add_row(Month = 1:3, 
          Sex = "Female") %>%
  complete(Month, Sex) %>% 
#  replace_na(list(Number = 0)) %>%  This line is incorrect, correct line added
  replace_na(list(count = 0)) %>%
  arrange(Sex, Month)

A <- ggplot(hedgehog_sum, aes(x = Month, y = count, color = Sex)) +
  geom_point() +
  geom_line() +
  theme_classic() +
...
C <- ggplot(hedgehog_sum2, aes(x = Month, y = count, color = Sex)) +
...
**Answer:** The list price is the explanatory variable. It helps us predict the response variable of Amazon price.
...
```{r}
ggplot(amazon_books, aes(x = list_price, y = amazon_price)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  theme_bw()
```

**Answer:** There is a fairly strong linear relationship between the two variables. As list price increases, the price on Amazon tends to increase as well. The points tend to fall pretty close to the regression line.
...
9. (4 points) Write a sentence interpreting the slope in context.

**Answer:** The predicted book price on Amazon increases by around 83 cents for every one dollar increase in the list price.

<br>


10. (4 points) Write a sentence interpreting the y-intercept in context.
