...
```{r}
majors_long <- mutate(majors_long, MajorsPer100 = Count / ArtsSciences * 100)
```

<br>


4. (4 points) Write code to get rid of the `ArtsSciences` variable from your `majors_long` data frame. Print the first 4 and last 4 rows of your new data frame.
...
5. (5 points) The code chunk below is set to `eval = FALSE`. Write code using the pipe operator that will accomplish questions 1 - 4 all in one step. In other words, you will only use the assignment operator once to go from reading your data in all the way to creating the data frame in Question 4.
...
**Your sentence:** The largest number of stats majors has been `r max(majors$Stats, na.rm = TRUE)` and the average number of math majors per year is `r mean(majors$Math)`.
...
8. (9 points) Now let's clean up the `billboard` data. Write code that does each of the following:
  ...
  - Convert the `Weeks` variable to numeric (Hint: You can use `mutate()` and a function mentioned in Notes 2. There will be a warning because the final song has an asterisk next to its number, so it will be converted to NA.)
...
9. (6 points) Write code that creates a data frame counting how many number one songs each artist had, then remove all artists with only 1 number one song, and sort the remaining artists in descending order by number of songs.
