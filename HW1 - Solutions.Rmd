...
**Answer:** We only need to install the package once. If we included code to install the package in this document, it would be installed every time we knitted our file.
...
**Answer:** The data has 1,340 observations that represent samples of coffee. Variables include things like country of origin and altitude at which it was grown (among many others).
...
str(coffee_ratings) # output hidden since it is VERY long
```

**Answer:** Possible examples are altitude, bag_weight, and harvest_year. Each of these are quantitative variables, but we would not be able to do numeric calculations for them as they are currently stored in the data frame.

**Note:** It doesn't look very good to have this long amount of output in the document. Usually, I would use a function like `str()` outside of my .Rmd file and then use what I learn to guide my approach to working with the data within the document.
...
coffee_sample <- head(coffee_ratings, n = 20)
coffee_sample$country_of_origin
```

**Answer:** In my 20 coffee samples, the median flavor score is `r median(coffee_sample$flavor)`.
...
prop.table(table(coffee_ratings$color))
```

**Answer:** No. There are supposed to be over 1,300 coffee samples, but my table only includes around 1,100 observations with color values.
...
```{r}
# Put your code below
table(coffee_ratings$color, useNA = "ifany")
prop.table(table(coffee_ratings$color, useNA = "ifany"))
```

**Answer:** Because my original tables didn't include a category for NA, the `useNA` default value must be `"no"`.
...
coffee_summary <- summarize(coffee_ratings,
                            max_points = max(total_cup_points),
                            min_acid = min(acidity),
                            avg_moisture = mean(moisture),
                            sd_aroma = sd(aroma, na.rm = TRUE),
...
```{r, echo = FALSE}
# Put your code below - I've added extra options to improve readability
...
beverages <- c("water", "tea", "coke", "juice")
```
