...
**Notes about object created:** It is a list, which is a new type of object for us. Lists can contain multiple other objects of different types. This one is pretty hard to tell what it contains.
...
```{r}
# pluck(1) gave table in top right of page, pluck(2) is the table I want
...
**Answer:** We changed the number in the `pluck()` function from 1 to 2. When it was one, we got the table in the top right of the Wikipedia page. Two gave us the main table we wanted. This may take some trial and error with future examples.

<br>


3. Look up the help file for the `rename()` function from the `dplyr` package. Use the function to rename the `Author(s)` column to just Author so that we get rid of the parentheses in the name. (Note: When referencing a column with special characters or spaces in it, you need to put tick marks around the name.)
...
```{r}
hugo_noms <- mutate(hugo_noms, Winner = str_detect(Author, "\\*"),
                    "NonWinner" = !str_detect(Author, "\\*"))
```
...
```{r}
hugo_noms <- mutate(hugo_noms, Author = str_remove(Author, "\\*"))
```
...
```{r}
hugo_winners <- filter(hugo_noms, Winner == TRUE)
```

<br>


7. Create a new object called `hugo_authors` that calculates the number of nominations (including wins) and number of wins for each author. Then sort the in descending order by number of nominations. Which author has been nominated the most for this award? Which author was nominated the most without ever winning? (Hint: In R, values of TRUE count as a 1 and values of FALSE count as a 0 in numeric calculations. To find the number of wins, use the `sum()` function.)
...
# Note: If you wanted to also sort by Wins, try:
#   arrange(desc(Nominations), desc(Wins))
```
